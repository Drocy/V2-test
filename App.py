...
DEFAULT_WEIGHTS = {
    ...
    "GOOGL": 0.10, "TSLA": 0.10, "BTC-USD": 0.10, "ETH-USD": 0.10
}
...
available = [x for x in DEFAULT_WEIGHTS if x in clean_data]

with st.sidebar.expander("Analysis controls", expanded=(page == "Portfolio Configuration")):
    window_label = st.selectbox("Analysis window", ["1Y", "2Y", "MAX"], index=1)
    confidence = st.selectbox("Tail-risk confidence", [0.95, 0.99], index=0,
                              format_func=lambda x: f"{x:.0%}")
    selected = st.multiselect("Portfolio assets", available, default=available)

    raw_weights = {}
    for ticker in selected:
        raw_weights[ticker] = st.number_input(
            f"{ticker} weight", min_value=0.0, max_value=1.0,
            value=float(DEFAULT_WEIGHTS.get(ticker, 0.0)), step=0.01,
            format="%.2f", key=f"w_{ticker}"
        )
    weight_sum = sum(raw_weights.values())
    if selected and weight_sum > 0:
        weights = {k: v / weight_sum for k, v in raw_weights.items()}
    else:
        weights = {}

prices_all = build_price_table(clean_data, FLAGSHIP)
if window_label == "1Y":
...
if pr.empty:
    st.error("No valid portfolio configuration is available.")
    st.stop()
...
st.sidebar.caption(f"Data through {prices_all.index.max():%Y-%m-%d}")
